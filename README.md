<h1 align="center">
  Hi 👋, I'm Hassan Raafat
</h1>

<h3 align="center">
Backend / Full-Stack Engineer • System Design • AI-Powered Products
</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=hassuunna&label=Profile%20views&color=0e75b6&style=flat" />
</p>

---

## 🧠 About Me

- 🧩 Backend engineer with **strong system ownership**
- 🏗️ Focused on **architecture, delivery, and long-term maintainability**
- 🧠 Experience across **web, desktop, enterprise automation, cloud & AI**
- 🔐 Security-aware, production-oriented mindset

---

## 🚀 Featured Projects

### 🏗️ Willma — Backend Platform

- 🔧 Designed backend architecture from scratch
- 🗄️ Major database restructuring & schema cleanup
- 🔄 Served **4 frontend apps** from a single backend
- 🚀 Owned production delivery end-to-end

**Tech:** Node.js, ORM, Relational DBs

---

### 🖥️ Nagwa Classes — Desktop App

- ⚡ Cross-platform desktop app using Electron + React
- 🔄 Feature development inside production codebase

---

### 🏢 ARIS Automation — STC

- 🤖 Monitoring & automation scripts
- 🧩 Automated Process Governance (APGs)
- 🔍 Fixed pipelines & production discrepancies
- 👥 Supported designers & process owners

---

### 🔐 Xifrem — Secure Chat (Open Source)

- 🔒 End-to-end encryption
- 🧼 Minimal & privacy-first design
- 🚀 Launched on Product Hunt
- 📊 Featured on SaaS platforms

---

### 🎬 Bta3 Aflam — AI Movie Bot

- 🤖 AI-powered movie search
- 🔗 Clean streaming & torrent links
- 😄 Humorous, engaging chatbot personality

---

## 🛠️ Tech Stack

### Backend

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=nodejs,express,nestjs,java,spring,postman,graphql,lambda&titles=true&theme=dark" />
</p>

### Frontend

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=react,reactnative,next,netlify,vite,tailwind,bootstrap,sass,css,html,jquery,figma&titles=true&theme=dark" />
</p>

### Desktop

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=electron,qt,cs&titles=true&theme=dark" />
</p>

### Databases

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=postgres,mysql,mongodb,sequelize,dbeaver&titles=true&theme=dark" />
</p>

### Cloud & DevOps

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=aws,azure,gcp,s3,heroku,jenkins,circleci,ngrok&titles=true&theme=dark" />
</p>

### Operating Systems & Tools

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=windows,linux,ubuntu,vscode,vscodeinsiders,wireshark,xd,idea,visualstudio&titles=true&theme=dark" />
</p>

---

## 🤖 AI & Automation

- 🧠 Prompt engineering
- 🧪 AI-assisted testing workflows
- 🤝 Multi-agent AI coordination
- ⚙️ Script-based automation & monitoring

---

## 🧠 Languages

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=js,ts,java,kotlin,python,cs,sql&titles=true&theme=dark" />
</p>

---

## 🧰 Development Tools

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=git,github,gitlab,githubcopilot,replit,npm,yarn,pnpm,eslint,prettier,postman,apidog,expo,jwt,markdown,regex&titles=true&theme=dark" />
</p>

---

## 🤖 AI & Collaboration

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=chatgpt,claude,gemini,githubcopilot,jira,miro,grafana,linkedin&titles=true&theme=dark" />
</p>

---

## 🌐 Browsers & Testing

<p>
  <img src="https://go-skill-icons.vercel.app/api/icons?i=chrome,brave,opera,firefox&titles=true&theme=dark" />
</p>

---

## 🧭 Philosophy

> Clean architecture.  
> Clear ownership.  
> Systems that scale without drama.
